...
from config import ROLLBACK_SCRIPT, UPGRADE_SCRIPT, STATE_ERROR, SSH_KEY_PATH, REMOTE_PORT, REMOTE_PASSWORD_TEST, FIXED_DEVICE_ID, READ_INPUT_REGISTER
...
def scp_send_async(src_file: Path, user: str, host: str, dest_dir: str):
    def _runner():
        try:
            ok, msg = scp_send(src_file, user, host, dest_dir, REMOTE_PASSWORD)
            DPrint(f"[SCP][async] host={host} ok={ok} msg={msg}")
        except Exception as e:
            DPrint(f"[SCP][async] host={host} exception: {e}")
        finally:
            global pending_count
            DPrint("locked =", pending_lock.locked())
            pending_lock.acquire()
            pending_count -= 1
            DPrint(f"[SCP][async] host={host} pending_count={pending_count}")
            if pending_count == 0:
                ...
                DPrint(f"[SCP][async] 警告: pending_count 负值: {pending_count}")
