...
import sys, time, ipaddress, configparser, struct, re, sqlite3, os, json, zipfile, shutil, threading, hashlib, tarfile
...
def get_job_upgrade_status(
    ...
    try:
        ...
        state = STATE_ERROR
        remark = data.get("msg", "状态异常，无法解析为整数")

    return progress, state, remark

def load_release_note(file_path: str = "info.json") -> Dict[str, Any]:
    """
    读取升级包release note json文件
    :param file_path: json文件路径，默认当前目录info.json
    :return: 解析后的完整字典数据
    """
    try:
        with open(file_path, "r", encoding="utf-8") as f:
...
def extract_inner_zip_by_prefix(
    ...
    """
    从经签名校验的总包中提取“文件名以 prefix 开头且以 .tar 结尾”的内层包。

    例如：
        prefix='LC'  可匹配 LC.tar / LC2026.tar / LCspc102.tar
        prefix='PCS' 可匹配 PCS.tar / PCS2026.tar

    只提取第一个匹配项。
    """
    prefix_lower = prefix.lower()

    with tarfile.open(outer_zip_path, 'r:gz') as tf:
        for member in tf.getmembers():
            ...
            if name_lower.startswith(prefix_lower) and name_lower.endswith('.tar'):
                out_dir.mkdir(parents=True, exist_ok=True)

                # 保留原始文件名，比如 LC2026.tar，不强制改名。
                safe_inner_name = secure_filename(name) or f"{prefix}.tar"
                dst = out_dir / safe_inner_name

                with tf.extractfile(member) as src, open(dst, 'wb') as f:
...
def extract_inner_tar(outer_package_path: Path, inner_tar_name: str, out_dir: Path) -> Path:
    """从已验签的 tar.gz 总包中提取指定名称的内层 tar。"""
    requested_name = Path(inner_tar_name).name
    requested_path = PurePosixPath(inner_tar_name)
    if (
        requested_path.is_absolute()
        or ".." in requested_path.parts
        or not requested_name.endswith(".tar")
    ):
        raise RuntimeError(f"无效的内层升级包名称: {inner_tar_name}")

    with tarfile.open(outer_package_path, "r:gz") as tf:
        matched = [
            member for member in tf.getmembers()
            if member.isfile() and member.name == inner_tar_name
        ]
        if len(matched) != 1:
            raise RuntimeError(f"升级包内未找到唯一的内层 tar: {inner_tar_name}")

        src = tf.extractfile(matched[0])
        if src is None:
            raise RuntimeError(f"无法读取内层升级包: {inner_tar_name}")
        out_dir.mkdir(parents=True, exist_ok=True)
        target = out_dir / requested_name
...
def safe_extract_tar(tar_file: tarfile.TarFile, dest_dir: Path):
    """安全解压 tar，仅允许目录和普通文件，拒绝链接及设备文件。"""
    for member in tar_file.getmembers():
        member_path = dest_dir / member.name
        if not _is_within_directory(dest_dir, member_path):
            raise RuntimeError(f"拒绝解压危险路径: {member.name}")

        if member.isdir():
            member_path.mkdir(parents=True, exist_ok=True)
        elif member.isfile():
            member_path.parent.mkdir(parents=True, exist_ok=True)
            with tar_file.extractfile(member) as src, open(member_path, 'wb') as dst:
                shutil.copyfileobj(src, dst)
        else:
            raise RuntimeError(f"拒绝解压非普通文件: {member.name}")
...
def parse_lc_ip_mapping_from_ini():
    try:
        ini_data = read_ini_all()
        lc_port_num = 4
        lc_ip_mapping = {}
        for i in range(1, lc_port_num + 1):
            ip = _ini_get_str(ini_data, f"NETWORK_net{i}", "ip", "")
            if ip:
                lc_ip_mapping[f"LC net{i}"] = ip
        return lc_ip_mapping
    except Exception as e:
        DPrint(f"[{now_str()}] 解析 ini 获取 LC IP 映射异常: {e}")
        return {}
...
def parse_device_ids_from_ips(ip_list):
    """
    根据设备IP列表解析设备ID列表
    ...
    lc_ip_mapping_list = parse_lc_ip_mapping_from_ini()
    # {
    #   "PCS1": "192.168.2.10",
    #   "PCS2": "192.168.2.11"
    # }
    pcs_ip_to_device_id = {
        ip: int(key.replace("PCS", ""))
        for key, ip in pcs_ip_mapping_list.items()
    }

    lc_ip_to_device_id = {
        ip: int(key.replace("LC net", ""))
        for key, ip in lc_ip_mapping_list.items()
    }

    for item in ip_list:
        # 支持 "ip1,ip2"
        for device_ip in item.split(","):
            device_ip = device_ip.strip()

            if not device_ip:
                continue

            if device_ip not in pcs_ip_to_device_id and device_ip not in lc_ip_to_device_id:
                raise ValueError(f"未找到对应 PCS 或 LC 设备: {device_ip}")

            if device_ip in lc_ip_to_device_id:
                device_ids.append(0)
            if device_ip in pcs_ip_to_device_id:
                device_ids.append(pcs_ip_to_device_id[device_ip])

    return device_ids

# ============ 升级包校验 =============
def extract_inner_tar_by_prefix(outer_tar_gz, prefix, work_dir):
    """
    从外层 .tar.gz 中找到唯一的业务 .tar，
    再从业务 .tar 中找到指定前缀的 .tar 文件，
    最终只将该文件提取到 work_dir，不进行二次解压。

    例如：

        upgrade_0909_signed.tar.gz
        ├── upgrade_0909.tar
        │   ├── info.json
        │   └── pcs123.tar
        └── upgrade_0909.tar.sig

    调用：

        pcs_tar = extract_inner_tar_by_prefix(
            outer_tar_gz,
            "pcs",
            work_dir
        )
    ...
    # ==================================================
    # 1. 外层 tar.gz 中必须存在且只能存在一个 .tar
    # ==================================================
    with tarfile.open(outer_tar_gz, "r:gz") as outer_tar:
        ...
        # 唯一的业务 tar
        business_tar_member = tar_members[0]

        # ==================================================
        # 2. 直接从内存文件对象打开业务 tar
        # ==================================================
        business_tar_file = outer_tar.extractfile(
            business_tar_member
        )

        if business_tar_file is None:
            raise RuntimeError(
                "无法读取业务 tar：{}".format(
                    business_tar_member.name
                )
            )

        business_tar_bytes = business_tar_file.read()
        with tarfile.open(fileobj=BytesIO(business_tar_bytes), mode="r:*") as business_tar:

            # ==================================================
            # 3. 在业务 tar 中寻找 prefix*.tar
            # ==================================================
            target_members = []

            for member in business_tar:
            ...
            if len(target_members) == 0:
                raise RuntimeError(
                    "业务 tar 中未找到 {}*.tar 文件".format(prefix)
                )

            # 找到多个
            if len(target_members) > 1:
                names = [
                    member.name for member in target_members
                ]

                raise RuntimeError(
                    "业务 tar 中存在多个 {}*.tar 文件：{}".format(
                        prefix,
                        names
                    )
                )
            ...
            target_file = business_tar.extractfile(
                target_member
            )
            ...
            with open(target_path, "wb") as f:
                while True:
                    data = target_file.read(1024 * 1024)
...
def verify_upgrade_package(pkg_file: Path, public_key_path: Path) -> str:
    """
    校验 tar.gz 升级包的内层 tar RSA-PSS-SHA256 签名。
    ...
    if not pkg_file.is_file():
        raise RuntimeError(f"升级包不存在: {pkg_file}")

    if not public_key_path.is_file():
        raise RuntimeError(f"公钥不存在: {public_key_path}")
    ...
    try:
        ...
        if openssl_version.returncode != 0:
            raise RuntimeError(
                f"openssl 无法正常运行: "
                f"{openssl_version.stderr.strip()}"
            )
    ...
    # ---------------------------------------------------------
    try:
        tf = tarfile.open(pkg_file, "r:gz")
    except Exception as exc:
        raise RuntimeError(
            f"无法打开升级包 {pkg_file}: {exc}"
        ) from exc
    ...
    with tf:
        ...
        if len(inner_members) != 1:
            raise RuntimeError(
                f"升级包内应当只有 1 个 .tar，"
                f"实际 {len(inner_members)} 个: "
                f"{[m.name for m in inner_members]}"
            )

        inner = inner_members[0]

        # -----------------------------------------------------
        # 5. 查找签名
        # -----------------------------------------------------
        sig_name = inner.name + ".sig"

        sig_members = [
            member
            for member in members
            if member.name == sig_name
        ]

        if len(sig_members) != 1:
            raise RuntimeError(
                f"升级包内应当有且只有 1 个签名文件: "
                f"{sig_name}，"
                f"实际 {len(sig_members)} 个"
            )
        ...
        with tempfile.TemporaryDirectory(
            prefix="upgrade_verify_"
        ) as tmp_dir:

            tmp_dir = Path(tmp_dir)

            # 只取 basename，避免 tar 内部路径影响临时文件路径
            inner_path = tmp_dir / Path(inner.name).name
            sig_path = tmp_dir / Path(sig_name).name

            # -------------------------------------------------
            # 7. 提取内层 tar
            # -------------------------------------------------
            inner_file = tf.extractfile(inner)

            if inner_file is None:
                raise RuntimeError(
                    f"无法读取内层升级包: {inner.name}"
                )

            with inner_file, open(inner_path, "wb") as fout:

                while True:
                    chunk = inner_file.read(1024 * 1024)
            ...
            # 8. 提取签名
            # -------------------------------------------------
            sig_file = tf.extractfile(sig_members[0])
            ...
            with sig_file, open(sig_path, "wb") as fout:
                fout.write(sig_file.read())
            ...
            if inner_size != inner.size:
                raise RuntimeError(
                    f"内层 tar 大小异常: "
                    f"tar 元数据={inner.size}, "
                    f"实际={inner_size}"
                )
            ...
            DPrint(f"[UPLOAD] 升级包签名校验成功: {pkg_file} -> {inner.name}")

    return inner.name

def read_upgrade_package_info(pkg_file: Path, inner_tar_name: str) -> dict:
    """从升级包的指定内层 tar 中读取并校验唯一的 info.json。"""
    requested_name = Path(inner_tar_name).name
    with tarfile.open(pkg_file, "r:gz") as outer_tf:
        inner_members = [
            member for member in outer_tf.getmembers()
            if member.isfile() and Path(member.name).name == requested_name
        ]
        if len(inner_members) != 1:
            raise RuntimeError(f"升级包内未找到唯一的内层 tar: {requested_name}")
        inner_stream = outer_tf.extractfile(inner_members[0])
        if inner_stream is None:
            raise RuntimeError(f"无法读取内层升级包: {requested_name}")

        with inner_stream, tarfile.open(fileobj=inner_stream, mode="r:") as inner_tf:
            members = inner_tf.getmembers()
            info_members = [
                member for member in members
                if member.isfile() and Path(member.name).name == "info.json"
            ]
            if len(info_members) != 1:
                raise RuntimeError(
                    f"内层升级包应当只有 1 个 info.json，实际 {len(info_members)} 个"
                )
            info_stream = inner_tf.extractfile(info_members[0])
            if info_stream is None:
                raise RuntimeError("无法读取升级包 info.json")
            with info_stream:
                try:
                    info = json.loads(info_stream.read().decode("utf-8"))
                except (UnicodeDecodeError, json.JSONDecodeError) as exc:
                    raise RuntimeError(f"升级包 info.json 格式错误: {exc}") from exc

    required_fields = {
        "firmware", "version", "file_name", "creator", "description",
        "deploy_profile", "scan_path",
    }
    missing = sorted(required_fields - set(info))
    if missing:
        raise RuntimeError(f"升级包 info.json 缺少字段: {', '.join(missing)}")
    if any(not isinstance(info[field], str) or not info[field].strip() for field in required_fields):
        raise RuntimeError("升级包 info.json 的必填字段必须为非空字符串")
    # info_filename = info["file_name"]
    # if info_filename != requested_name:
    #     raise RuntimeError(f"升级包 info.json 的 file_name({info_filename}) 与内层 tar({requested_name}) 名称不一致")
    # 查找实际业务 tar
    business_members = [
        member for member in members
        if member.isfile()
        and Path(member.name).suffix.lower() == ".tar"
        and Path(member.name).name != requested_name
    ]

    if len(business_members) != 1:
        raise RuntimeError(
            f"内层升级包应当只有 1 个业务 tar，实际 {len(business_members)} 个"
        )

    actual_business_name = Path(business_members[0].name).name

    # 校验 file_name
    info_filename = info["file_name"].strip()

    if info_filename != actual_business_name:
        raise RuntimeError(
            f"升级包 info.json 的 file_name({info_filename}) "
            f"与实际业务包({actual_business_name})名称不一致"
        )

    # scan_path = PurePosixPath(info["scan_path"])
    # if scan_path.is_absolute() or ".." in scan_path.parts:
    #     raise RuntimeError("升级包 info.json 的 scan_path 必须为安全的相对路径")
    return {field: info[field].strip() for field in required_fields}
...
def _record_upgrade_operation(operator_name, operation_target, result, remark):
    """异步写入升级操作日志；日志失败不影响升级接口的业务响应。"""
    operator_name = operator_name or "unknown"
    remark = str(remark)[:255]
    queued = operationLog.enqueue_operation_log(
        operator_name,
        operationLog.OperationType.UPGRADE,
        operation_target[:64],
        1,
        result,
        remark,
    ...
    DPrint(
        f"[UPGRADE][OPLOG] operator={operator_name}, target={operation_target}, "
        f"result={result.name}, queued={queued}, remark={remark}"
    )
...
def write_register_with_retry(write_address: int, write_value: int, slave: int = 1):
    ...
    with remote_client_lock:
        ...
        uint_value = int16_to_uint16(write_value)

        result = remote_client.write_register(write_address, uint_value, slave=slave)
        if result is not None and (not result.isError()):
            DPrint(f"[{now_str()}] [REMOTE] 下发寄存器成功: addr={write_address}, value={uint_value}")
            return result, 1
        ...
        result2 = remote_client.write_register(write_address, uint_value, slave=slave)
        DPrint(f"[{now_str()}] [REMOTE] 失败后再次下发寄存器: addr={write_address}, result={result}")
        return result2, 2
